...
void main() {
  ...
  test('simulate recording writes CSV files into selected folder', () async {
    ...
    final vm = SerialConnectionViewModel(
      serialFactory: (port, baud, {simulate = false}) {
        return SerialSource(port, baud, simulate: true); // TEST: always simulated
      },
    );
    ...
    expect(err, isNull, reason: 'Connect (including simulation fallback) should succeed');
    ...
    expect(files.isNotEmpty, isTrue, reason: 'At least one CSV file should be created');

    final content = files.first.readAsStringSync();
    expect(content.contains('timestamp'), isTrue, reason: 'CSV header should contain timestamp');
