...
class FakeSerialSource extends SerialSource {
  ...
  FakeSerialSource(String portName, int baudRate, {bool simulate = false, bool connectResult = false})
      : _connectResult = connectResult,
        super(portName, baudRate, simulate: simulate);
...
void main() {
  ...
  test('no simulation by default when connection fails', () async {
    final vm = SerialConnectionViewModel(
      serialFactory: (port, baud, {simulate = false}) {
        ...
        return FakeSerialSource(port, baud, simulate: simulate, connectResult: false);
      },
    );
  ...
  test('simulation only when injected (factory returns simulated source)', () async {
    ...
    expect(files.isNotEmpty, isTrue,
        reason: 'CSV file should be created when simulation is active and folder is set');
